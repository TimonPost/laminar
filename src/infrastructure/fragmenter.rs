...
use crate::packet::header::FragmentHeader;
...
impl Fragmentation {
    ...
    pub fn spit_into_fragments<'a>(
        ...
        let num_fragments =
            Fragmentation::fragments_needed(payload_length, config.fragment_size) as u8; /* safe cast max fragments is u8 */
    ...
    fn create_fragment_if_not_exists(&mut self, fragment_header: FragmentHeader) {
        if !self.fragments.exists(fragment_header.sequence()) {
            let reassembly_data = ReassemblyData::new(
                ...
                (9 + self.config.fragment_size) as usize, // todo: check this
            );
