...
pub enum SocketEvent {
    ...
    ///
    /// On the server—in order to receive a `Connect` event—you must respond to the the first
    ...
    Timeout(SocketAddr),
    /// The client has timed out that we had an established conncetion with.
