use super::{ConnectionTimeout, VirtualConnection};
...
pub struct ConnectionPool
{
    ...
    connections: Arc<RwLock<HashMap<SocketAddr, Arc<RwLock<VirtualConnection>>>>>,
...
impl ConnectionPool {
    pub fn new() -> ConnectionPool
    {
        ...
        ConnectionPool { timeout: TIMEOUT_DEFAULT, connections: Arc::new(RwLock::new(HashMap::new())), sleepy_time, poll_interval }
    }

    /// Set the timeout before an client will be seen as disconnected.
    pub fn set_timeout(&mut self, timeout: ConnectionTimeout)
    {
        self.timeout = timeout;
    }

    /// Insert connection if it does not exists.
    pub fn insert_if_not_exists(&mut self, addr: &SocketAddr) -> Result<Arc<RwLock<VirtualConnection>>> {
        let mut lock = self
            .connections
            .write()
    ...
    pub fn start_time_out_loop(&self) -> Result<thread::JoinHandle<()>>
    {
            ...
            .spawn(move ||
                loop {
                    match connections.write() {
                        Ok(mut lock) => {
                            Self::check_for_timeouts(&mut *lock, poll_interval);
                        },
                        Err(e) => {
    ...
    fn check_for_timeouts(connections: &mut HashMap<SocketAddr, Arc<RwLock<VirtualConnection>>>, sleepy_time: Duration) {
