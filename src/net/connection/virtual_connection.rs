use crate::{
    ...
    packet::{
        DeliveryGuarantee, Fragment, OrderingGuarantee, Outgoing, OutgoingPacketBuilder, Packet,
...
pub struct VirtualConnection {
    ...
    congestion_hanlder: CongestionHandler,
...
impl VirtualConnection {
    ...
    pub fn new(addr: SocketAddr, config: &Config) -> VirtualConnection {
        VirtualConnection {
            ...
            congestion_hanlder: CongestionHandler::new(config),
        ...
        match delivery_guarantee {
            DeliveryGuarantee::Unreliable => {
                ...
                if let OrderingGuarantee::Sequenced(stream_id) = ordering_guarantee {
                    let item_indentiefier = self
                        .sequencing_system
                        .get_or_create_stream(stream_id.unwrap_or(DEFAULT_SEQUENCING_STREAM))
                        .new_item_identifier();

                    builder = builder.with_sequencing_header(item_indentiefier as u16, stream_id);
                };
            ...
            DeliveryGuarantee::Reliable => {
                ...
                let outgoing = {
                    ...
                    if payload_length <= self.config.fragment_size {
                        let mut builder = OutgoingPacketBuilder::new(payload)
                            .with_default_header(
                                PacketType::Packet,
                                delivery_guarantee,
                                ordering_guarantee,
                            )
                            .with_acknowledgement_header(
                ...
                self.congestion_hanlder
                    .outgoing(self.acknowledge_handler.seq_num);
                self.acknowledge_handler.outgoing(payload);
            ...
            DeliveryGuarantee::Reliable => {
                if header.is_fragment() {
                    if let Ok((fragment_header, acked_header)) = packet_reader.read_fragment() {
                        ...
                        {
                            Ok(Some(payload)) => {
                                sender.send(SocketEvent::Fragment(Fragment::new(
                        ...
                        if let Some(acked_header) = acked_header {
                            self.congestion_hanlder.incoming(acked_header.sequence());
                            self.acknowledge_handler.incoming(acked_header.sequence());
                        }
                    }
                } else {
...
mod tests {
    ...
    fn assure_right_fragmentation() {
        ...
        protocol_version
            .write_u32::<BigEndian>(ProtocolVersion::get_crc32())
            .unwrap();
        ...
        match complete_fragment {
            SocketEvent::Fragment(fragment) => assert_eq!(
                fragment.payload(),
    ...
    fn assert_incoming_with_order(
        ...
        header.parse(&mut packet);

        if let OrderingGuarantee::Sequenced(val) = ordering {
            if delivery == DeliveryGuarantee::Unreliable {
