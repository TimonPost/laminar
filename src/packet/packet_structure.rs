...
mod tests {
    ...
    fn create_unreliable() {
        let header = Packet::unreliable(test_addr(), test_payload());

        assert_eq!(header.addr(), test_addr());
        assert_eq!(header.payload(), test_payload().as_slice());
        assert_eq!(header.delivery_guarantee(), DeliveryGuarantee::Unreliable);
        assert_eq!(header.order_guarantee(), OrderingGuarantee::None);
    }

    #[test]
    fn create_unreliable_sequenced() {
        let header = Packet::unreliable_sequenced(test_addr(), test_payload(), Some(1));

        assert_eq!(header.addr(), test_addr());
        assert_eq!(header.payload(), test_payload().as_slice());
        assert_eq!(header.delivery_guarantee(), DeliveryGuarantee::Unreliable);
        assert_eq!(
            header.order_guarantee(),
    ...
    #[test]
    fn create_reliable() {
        let header = Packet::reliable_unordered(test_addr(), test_payload());
    ...
    fn create_reliable_ordered() {
        let header = Packet::reliable_ordered(test_addr(), test_payload(), Some(1));
