...
use self::laminar::packet::header::{PacketHeader, FragmentHeader, HeaderParser, HeaderReader};
...
fn send_benchmark(c: &mut Criterion)
{
    ...
    c.bench_function("process send", move |b| b.iter(|| process_packet_before_send(&mut socket_state, &config)));
}

fn process_packet_when_received(packet_processor: &mut PacketProcessor, data: &Vec<u8>, socket_state: &mut SocketState)
{
    let packet = packet_processor.process_data(data.clone(), CLIENT_ADDR.parse().unwrap(), socket_state).unwrap().unwrap();
}
...
fn receive_benchmark(c: &mut Criterion)
{
    ...
    c.bench_function("process received",  move |b| b.iter(|| process_packet_when_received(&mut packet_processor, &parsed, &mut socket_state)));
}
